...
# find_traceable_tx looks for a recent transaction to trace, in one batch so a
# remote endpoint is not hit with a round trip per block. Leaves TRACE_TX empty
# when the scanned range holds no transactions.
TRACE_TX=""
find_traceable_tx() {
	local head scan i batch="" out
	...
	for ((i = 0; i < scan; i++)); do
		[ -n "$batch" ] && batch="$batch,"
		batch="$batch$(printf '{"jsonrpc":"2.0","method":"eth_getBlockByNumber","params":["0x%x",false],"id":%d}' $((head - i)) "$i")"
	done
	out=$(rpc_call "[$batch]")
...
check_batch "11 traces rejected" \
	"$(build_batch 11 "$trace_elem")" \
	-32600 "too many trace calls in batch: 11 exceed the limit of 10"

# At the limit the batch must be let through. What each call then returns is a
# property of the chain, so the assertion is only that nothing was rejected by
# the batch cap.
resp=$(rpc_call "$(build_batch 10 "$trace_elem")")
if [ "$(printf '%s' "$resp" | jq '[.[] | select(.error.code == -32600)] | length')" = "0" ]; then
	ok "10 traces accepted (at the limit)"
else
	bad "10 traces rejected by the batch cap" "$resp"
fi

check_batch "101 requests rejected" \
	"$(build_batch 101 "$plain_elem")" \
	-32600 "batch too large: 101 requests exceed the limit of 100"

resp=$(rpc_call "$(build_batch 100 "$plain_elem")")
if [ "$(printf '%s' "$resp" | jq '[.[] | select(has("result"))] | length')" = "100" ]; then
	ok "100 requests accepted (at the limit)"
else
	bad "100 requests rejected" "$resp"
fi
echo
