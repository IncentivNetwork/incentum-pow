...
func (ethash *Ethash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool, unixNow int64) error {
	...
	if !chain.Config().IsLondon(header.Number) {
		...
		// Verify the header's EIP-1559 attributes. stateDB is nil because
		// parent post-state is not available here; the dynamic min base fee
		// floor is enforced later in core.StateProcessor.Process where parent
		// state is attached.
		return err
	}
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	switch {
	case config.IsFast(next):
		return calcDifficultyFast(time, parent)
...
// calcDifficultyFast is the difficulty adjustment algorithm for fast consensus.
// It returns the difficulty that a new block should have when created at time given the
// parent block's time and difficulty. The calculation uses modified Homestead rules
// with 5-second target block time and NO difficulty bomb for long-term stability.
func calcDifficultyFast(time uint64, parent *types.Header) *big.Int {
	// https://github.com/ethereum/EIPs/blob/master/EIPS/eip-2.md
	// algorithm (modified for precise 5-second target, no bomb):
	// diff = (parent_diff +
...
var FastDifficultyCalculator = calcDifficultyFast
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if config.IsZeroReward(header.Number) {
